        ...
        <>
            ...
            {dummyData.map((data: any, index: any) => (
                <div className='grid grid-flow-col grid-cols-6 divide-x-[1px] border-b-[1px]' key={index}>
                    ...
                    <div className='col-span-1 px-2 py-1 text-sm flex gap-2'>{data.categories.map((val: string, index: number) => (<span key={index} className={`border-[1px] border-${data.categoriesclr[index]}-200  bg-${data.nextMeetingClr}-100 border-[1px] bg-gray-100 px-1 py-0.5 rounded-md`}>{val}</span>))}</div>
                    ...
                    <div className='col-span-1 px-2 py-1 text-sm'><span className={`border-[1px] border-${data.nextMeetingClr}-200  bg-${data.nextMeetingClr}-100 text-${data.nextMeetingClr}-900 px-1 py-0.5 rounded-md`}>{data.nextMeeting}</span></div>
                    <div className='px-2'><i className="ri-add-line text-base text-white"></i></div>
                </div >
                // <span className="border-[1px] bg-gray-100 px-1 py-0.5 rounded-md">{data.categories}</span>
            ))}
